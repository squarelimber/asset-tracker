...
- 份额型平滑持仓（银行理财等手动净值产品）的历史估值改为按买卖流水回放份额与成本。以前任意一天都用「当前份数」，部分赎回后一旦全量重建历史，赎回日之前的每一天都会被按赎回后的份数重估，整条净值曲线塌掉（月月宝赎回 107k 份 → 曲线掉约 11 万）。本次同时修回填、日明细、产品收益日历三处口径，并新增一次性重建标记让老库装上后自动修复已塌陷的曲线
- 变更持仓所属账户不再引发任何数值变化（账户只是容器；此前是回填全量重建放大了旧缺陷）
